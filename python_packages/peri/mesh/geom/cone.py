"""
This is python package for Peridigm Mesh Generation(Cone).
"""
import math
import random
import numpy as np
...
def sample():
    return {'radius':7.6,'height':40,'res': 1 , 'id': 2,'center':[0,0,0],'rand':0.03}
...
def create(prop={}):
    ...
    ------------------
    """
    prop_ = {'radius':7.6,'height':40,'res': 1 , 'id': 2,'center':[0,0,0],'rand':0.03}
    ...
    mask = []
    for i in data:
        if (i[0]-center[0])**2+(i[1]-center[1])**2 < (i[2]-center[2])**2 /(length)**2 * radius**2:
            mask.append(True)
        else:
            mask.append(False)
