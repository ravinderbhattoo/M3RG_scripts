function [timestep,box,id,label,coordinate,charge,varargout]=read_trjq(fd,flag_sort,varargin)
...
if ~strcmp(temps,'ITEM: BOX BOUNDS xy xz yz pp pp pp')
    ...
    if length(varargin)>1
        ...
        for s=1:na
            ...
            charge(s,:)=frame{11+6*(s-1)};
            coordinate(s,1,:)=frame{12+6*(s-1)};
            coordinate(s,2,:)=frame{13+6*(s-1)};
            coordinate(s,3,:)=frame{14+6*(s-1)};
        end
        box=zeros(3,3,nframe);
        box(1,1,:)=frame{4}-frame{3};
        box(2,2,:)=frame{6}-frame{5};
    ...
    if length(varargin)>1
        ...
        for s=1:na
            ...
            charge(s,:)=frame{14+6*(s-1)};
            coordinate(s,1,:)=frame{15+6*(s-1)};
            coordinate(s,2,:)=frame{16+6*(s-1)};
            coordinate(s,3,:)=frame{17+6*(s-1)};
        end
        box=zeros(3,3,nframe);
        xy=frame{5}';
        xz=frame{8}';
