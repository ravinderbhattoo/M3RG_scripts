function [timestep,box,id,label,coordinate,disp,varargout]=read_trj_disp(fd,flag_sort,varargin)
...
if ~strcmp(temps,'ITEM: BOX BOUNDS xy xz yz pp pp pp')
    ...
    if length(varargin)>1
        ...
        frame=textscan(fd,['ITEM: TIMESTEP\n%f\nITEM: NUMBER OF ATOMS\n%f\nITEM: BOX BOUNDS xy xz yz pp pp pp\n%f%f%f\n%f%f%f\n%f%f%f\nITEM: ATOMS id type x y z c_disp \n',repmat(datastring,1,na)]);
        ...
        for s=1:na
            ...
            disp(s,:)=frame{17+5*(s-1)};
            flags(s,1,:)=frame{18+5*(s-1)};
            flags(s,2,:)=frame{19+5*(s-1)};
            flags(s,3,:)=frame{20+5*(s-1)};
        end
        box=zeros(3,3,nframe);
        xy=frame{5}';
        xz=frame{8}';
