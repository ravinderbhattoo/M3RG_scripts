function [timestep,box,id,label,coordinate,varargout]=read_lammpstrj(fd,flag_sort,varargin)
...
if ~strcmp(temps,'ITEM: BOX BOUNDS xy xz yz pp pp pp')
    ...
    if length(varargin)>1
        ...
        for s=1:na
            id(s,:)=frame{9+5*(s-1)};
            label(s,:)=frame{10+5*(s-1)};
            coordinate(s,1,:)=frame{11+5*(s-1)};
            coordinate(s,2,:)=frame{12+5*(s-1)};
            coordinate(s,3,:)=frame{13+5*(s-1)};
        end
        box=zeros(3,3,nframe);
        box(1,1,:)=frame{4}-frame{3};
        box(2,2,:)=frame{6}-frame{5};
